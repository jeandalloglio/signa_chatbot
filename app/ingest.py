...
def chunk_text(t: str, chunk_size: int, overlap: int):
    ...
    while start < len(t):
        ...
        c = t[start:end]
        # tenta cortar no próximo parágrafo, se estiver perto
        if end < len(t):
            next_nl = t.find("\n\n", end)
...
async def fetch(client: httpx.AsyncClient, url: str) -> str:
    """
    Fetch com retry rápido e validação de conteúdo HTML.
    Usa timeouts/limits definidos no cliente.
    """
    last_err = None
    for _ in range(2):  # 1 retry
        try:
            ...
            if "text/html" not in ctype:
                raise RuntimeError(f"conteúdo não-HTML ({ctype})")
            return r.text
...
async def crawl(seed_urls, max_pages=150):
    ...
    # timeouts mais curtos e limits para evitar travar
    timeout = httpx.Timeout(connect=5.0, read=10.0, write=10.0, pool=5.0)
    limits = httpx.Limits(max_keepalive_connections=5, max_connections=10)

    async with httpx.AsyncClient(
        follow_redirects=True,
        headers={"User-Agent": "SignaChatbot/1.0"},
        timeout=timeout,
        limits=limits,
    ) as client:
        ...
        while queue and len(pages) < max_pages:
            ...
            try:
                ...
                # pula rápido páginas problemáticas
                continue

            pages.append({"url": url, "html": html})
            pbar.update(1)

            # coleta próximos links
            soup = BeautifulSoup(html, "html.parser")
